...
export async function iniciarTorneo(id){
    //Necesita token
    const token=localStorage.getItem('token');
    const respuesta=await fetch(`${URL_BASE}/api/torneos/${id}/generar-calendario`,{
        method:'POST',
        headers:{
            'Content-Type':'application/json',
            'Authorization':`Bearer ${token}`,
...
export async function eliminarEquipo(torneoId,equipoId){
    //Necesita token
    const token=localStorage.getItem('token');
    const respuesta=await fetch(`${URL_BASE}/api/torneos/${torneoId}/equipos/${equipoId}`,{
        method:'DELETE',
        headers:{
            'Content-Type':'application/json',
            'Authorization':`Bearer ${token}`
...
export async function obtenerJornadasDelTorneo(torneoId){
    const respuesta=await fetch(`${URL_BASE}/api/torneos/${torneoId}/jornadas`,{
        method:'GET',
        headers:{
            'Content-Type':'application/json'
        }
...
export async function obtenerPosicionesDelTorneo(torneoId){
    const respuesta=await fetch(`${URL_BASE}/api/torneos/${torneoId}/posiciones`,{
        method:'GET',
        headers:{
            'Content-Type':'application/json'
        }
    ...
    if (!respuesta.ok) {
       const mensaje = Array.isArray(datos.detail)
         ? datos.detail.map((d) => d.msg).join(", ")
         : datos.detail;
       throw new Error(mensaje || "Error al obtener al obtener las posiciones del torneo.");
     }
...
export async function editarMarcador(partidoId,payload){
    ...
    if (!respuesta.ok) {
      const mensaje = Array.isArray(datos.detail)
        ...
        : datos.detail;
      throw new Error(
        mensaje || "Error al modificar el resultado del partido.",
      );
    }
